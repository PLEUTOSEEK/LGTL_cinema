    ...
    if (!is_null($custDtls)) {
        ...
        if (date("Y-m-d") > date("Y-m-d", strtotime($orderDate . ' +3 day'))) {
            ...
            $result = array(
                ...
                "msgBody" => nl2br("Dear " . $custDtls['CUST_NAME'] . ":"
                        . "\n\nGreetings, we have received the request for a refund from you for order ID [$orderID]. According to our company policy, we are able to make refunds for you if the order date has within 3 issue days. Hence, we are here to inform you that your refund request has been approved. Please be patient, we are processing your refunds request. "
                        . "\n\nYou are always welcome to email us via our contact channel. Stay tuned."
                        . "\n\nBest Regards,"
                        . "\nLGTL Cineplex"
                        . "\n\n\n*This email is auto-generated by the system, please do not make any reply.*")
...
function setSeatsAvailable($orderID) {
    $conn = OpenCon();
    ...
    $sql = "UPDATE
            ...
            SET
                STATUS = 'Available'
            WHERE
                SCHEDULE_ID IN (SELECT SCHEDULE_ID FROM ORDER_LIST WHERE SCHEDULE_ID IS NOT NULL AND ORDER_ID = ?);";
...
function setOrderRefunded($orderID) {
    $conn = OpenCon();
